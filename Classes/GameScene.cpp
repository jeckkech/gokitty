...
bool GameScene::init()
{
	...
	actionArrowSprite = CCSprite::create("common/arrow.png");
	actionArrowSprite->setPosition(Point(visibleSizeWidth / 2, visibleSizeHeight / 5 * 4));
	actionArrowSprite->setVisible(false);
	this->addChild(actionArrowSprite, 1000);
...
void GameScene::update(float dt) {
	...
	for (int i = 0; i < columnList.size(); i++) {
		...
		if (diff > 0 && diff <= (columnList.at(i)->getContentSize().width * colScale * 4)) {
			if(!actionArrowSprite->isVisible()){
				int direction = cocos2d::random(0, 3);
			
				if (direction == UP) {
					actionArrowSprite->setRotation(-90);
					actionArrowSprite->setVisible(true);
					...
					actionArrowSprite->setRotation(180);
					actionArrowSprite->setVisible(true);
					moveDirection = direction;
				}
				else if (direction == RIGHT) {
					actionArrowSprite->setRotation(0);
					actionArrowSprite->setVisible(true);
					moveDirection = direction;
				}
				else {
